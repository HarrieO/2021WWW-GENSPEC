...
def add_clicks(click_list, safe_copy=False):
  if safe_copy:
    result = click_list[0].copy()
    result['num_clicks'] = 0
    if 'click_patterns' in result:
      result['click_patterns'] = {}
    for k in ['query_freq', 'doc_position_display_freq', 'clicks_per_doc']:
      result[k] = np.zeros_like(result[k])
    to_add = click_list
  else:
    result = click_list[0]
    to_add = click_list[1:]
  for clicks in to_add:
